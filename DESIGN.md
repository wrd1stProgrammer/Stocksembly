...
- Personal work seats use `(seat)/(input)/facing`: Maya `(4,13)/(4,14)/down`; June `(8,13)/(8,14)/down`; Alex `(12,13)/(12,14)/down`; Ethan `(31,13)/(31,14)/down`; Aria `(35,13)/(35,14)/down`; Leo `(39,13)/(39,14)/down`; Noah `(5,30)/(5,31)/down`; Sofia `(9,30)/(9,31)/down`; Hana `(13,30)/(13,31)/down`; Liam `(31,30)/(31,31)/down`; Min `(35,30)/(35,31)/down`. Navigation retains these safely separated cells, while each rendered chair and seated actor aligns to the center of one monitor inside the generated bitmap's visible content bounds, excluding transparent side padding. The workstation bitmap is visually lowered by `8px`; its separate chair and seated actor are lowered by `16px`, keeping them tight to the desk without crowding labels or routes. Meeting seats retain the current table composition: Maya `(8,7)/(8,8)/down`; June `(7,11)/(7,10)/up`; Alex `(9,11)/(9,10)/up`; Ethan `(35,7)/(35,8)/down`; Aria `(34,11)/(34,10)/up`; Leo `(37,11)/(37,10)/up`; Noah `(7,24)/(7,25)/down`; Sofia `(11,24)/(11,25)/down`; Hana `(9,28)/(9,27)/up`; Liam `(31,24)/(31,25)/down`; Min `(34,28)/(34,27)/up`. Dr. Park remains `(22,11)/(22,10)/up`.
...
- Department furniture is generated from the same manifest as nine independent Pixi clusters: four compact image-generated personal workstation banks, four compact procedural meeting tables, and Dr. Park's chair desk. Every personal bank sits in the lower band of its department and encodes the actual headcount (Market/Company/Financial `3`, Risk `2`) as equal-width desk-and-monitor modules. The generated bitmap contains only desks and monitors: no chair, person, wall, room background, floor shadow, plant, stationery scatter, or photographic clutter. Its simplified top-down game-entity material, outline weight, monitor scale, and shallow depth match the existing office furniture. Every analyst sits on the north side of the bank with `facing: down`, so the front-facing seated frame remains visible. Existing direction-specific chair sprites stay separate and are the only personal chairs. Meeting tables keep their own persistent chairs and tabletop laptops. Furniture entries declare `purpose: workstation | meeting | chair`, and only the matching seat group is attached to that cluster. Furniture footprints are physical obstacles; every personal seat, meeting seat, door, visitor anchor, and cross-room route remains walkable and unobstructed.
