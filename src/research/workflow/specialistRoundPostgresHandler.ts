...
import type { specialistPromptRequest } from "./specialistRoundPostgresStage";
...
async function attemptRole(
  ...
  const replay = await authority.replay(attempt.runId);
  return (
...
export function createSpecialistRoundAttemptHandler(
  ...
  const recordEvidence = async (
    ...
    await recordSuccessfulRunnerEvidence(
      context.commitStore,
      {
        runId: attempt.runId,
        jobId: attempt.jobId,
  ...
  const execute = async (
    ...
    const job = await context.authority.loadJob(
      attempt.runId,
      `memo:${await attemptRole(attempt, context.authority)}`,
    );
    const claim = await context.authority.claimForAttempt(attempt.attemptId);
    let attemptInputHash = await context.authority.inputHashForAttempt(
      attempt.attemptId,
    );
    if (
      job === undefined ||
    ...
    const previousFeedback = await context.authority.previousValidationFeedback(
      attempt.attemptId,
    );
    const durablePrompt =
      previousFeedback === undefined
        ? ((await context.authority.repairPromptForInput(
            attempt.jobId,
            attemptInputHash,
          )) ??
        ...
        : {
            validationCode: await context.authority.retryCodeForJob(
              attempt.jobId,
            ),
            prompt: `${specialistValidationCorrectivePrompt(job.prompt, await context.authority.retryCodeForJob(attempt.jobId))}
...
Repair the specific claims above instead of regenerating the same invalid numbers. For a percentage absent from the exact registered concept and fiscal period, retain the source-backed mechanism and absolute operating KPI, but express that rate directionally. A percentage appearing in filing prose alone does not create a registered metric. Return the full corrected memo. Keep English fields in English and Korean fields in Korean.`,
    ...
    if (
      ...
      if (
        !(await context.authority.persistRepairPrompt({
          jobId: attempt.jobId,
      ...
      if (
        ...
        !(await context.authority.rebindReplacementInput(
          attempt.attemptId,
          correctedInputHash,
        ))
      )
    ...
    try {
      ...
      for (const source of (
        await context.authority.sourceArtifactsForJob(job.jobId)
      ).filter((artifact) =>
    ...
    if (!claimValidation.ok) {
      await context.authority.persistValidationFeedback(
        attempt.attemptId,
    ...
    if (
      ...
      departmentId === "chair" ||
      !(await context.authority.reserveDepartmentTheses({
        runId: job.runId,
        departmentId,
        roleId: job.roleId,
    ...
    if (committed.kind === "citation_replacement_reserved") {
      ...
      if (
        ...
        return "incomplete";
      await context.authority.consumeReplacementBudget(attempt.runId);
      await context.authority.markReplacementRunning(ids.replacementAttemptId);
