import { normalizeEditorialText, textSimilarity } from "../domain/editorialQuality";
...
function hasOnlyGroundedNumbers(summary: string, sources: readonly string[]): boolean {
...
function sharesGroundingLanguage(summary: string, sources: readonly string[]): boolean {
  const sourceTokens = new Set(
    sources.join(" ").toLocaleLowerCase("und").match(/[\p{L}\p{N}_-]{2,}/gu) ?? [],
  );
  const summaryTokens = summary.toLocaleLowerCase("und").match(/[\p{L}\p{N}_-]{2,}/gu) ?? [];
...
export function publicTextIsValid(
  ...
  if (!/\p{Script=Latin}/u.test(text.en) || !/\p{Script=Hangul}/u.test(text.ko)) return false;
...
export function decisionTextsAreDistinct(texts: readonly PublicText[]): boolean {
...
export function isSymmetricHedge(text: PublicText): boolean {
  ...
  const pairedEn = /\b(?:upside|reward|opportunity)s?\b.*\b(?:downside|risk)s?\b|\b(?:downside|risk)s?\b.*\b(?:upside|reward|opportunity)s?\b/u.test(en);
  const balancedEn = /\b(?:equal|equally|balanced|same|symmetric|symmetrical)\b/u.test(en);
  const pairedKo = /(?:상방|상승|기회).*(?:하방|하락|위험)|(?:하방|하락|위험).*(?:상방|상승|기회)/u.test(ko);
  const balancedKo = /(?:동일|같(?:다|은|이|게)?|균형|대등|비슷)/u.test(ko);
  return (
    /\b(?:could|may)\s+(?:rise|increase).{0,48}\b(?:could|may)\s+(?:fall|decline)\b/iu.test(text.en) ||
