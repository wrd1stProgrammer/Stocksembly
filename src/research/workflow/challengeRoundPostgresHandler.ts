...
import type { PostgresChallengeRoundOptions } from "./challengeRoundContracts";
import { ChallengeDecisionSchema } from "./challengeRoundContracts";
import { inspectChallengeCandidate } from "./challengeRoundOutput";
import type { ChallengeRoundPostgresAuthority } from "./challengeRoundPostgresAuthority";
...
type HandlerContext = {
  ...
  readonly challengeAuthority: ChallengeRoundPostgresAuthority;
...
export function createChallengeRoundAttemptHandler(
  ...
  const execute = async (
    ...
    const logicalArtifactId =
      await context.workflowAuthority.logicalArtifactForAttempt(
        attempt.attemptId,
      );
    const job =
      logicalArtifactId === undefined
        ? undefined
        : await context.challengeAuthority.loadJob(
            attempt.runId,
            logicalArtifactId,
          );
