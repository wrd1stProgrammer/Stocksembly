...
async function workflowRows(database: ResearchDatabase, runId: string) {
  return (
    await database.query(
      `SELECT artifacts.artifact_id, artifacts.logical_key,
      artifacts.content_hash, artifacts.created_at
    FROM artifacts
    WHERE artifacts.run_id = $1 AND (
...
export async function buildOfficialStructuralAuditInput(options: {
  ...
  const snapshot = SnapshotRowSchema.parse(
    (
      await database.query(
        `SELECT snapshots.snapshot_id, snapshots.requested_at,
          snapshots.evidence_cutoff_at FROM runs
        JOIN snapshots ON snapshots.snapshot_id = runs.snapshot_id
        WHERE runs.run_id = $1 AND snapshots.state = 'sealed'`,
        [runId],
      )
    ).rows[0],
  );
  const accepted = await workflowRows(database, runId);
  ...
  const evidenceRows = (
    await database.query(
      `SELECT DISTINCT artifacts.artifact_id, artifacts.logical_key,
        artifacts.content_hash, artifacts.created_at,
      ...
      WHERE artifacts.run_id = $1 AND (
        artifacts.logical_key LIKE 'evidence:%' OR artifacts.artifact_id IN (
          ...
          WHERE child.run_id = $2 AND child.logical_key LIKE 'memo:%'
  ...
  ).rows.map((row) => EvidenceArtifactRowSchema.parse(row));
