...
import type { SpecialistSourceArtifact } from "../workflow/specialistRoundPostgresContracts";
...
export async function planResearchBrief(
  ...
  const saved = (
    await database.query(
      "SELECT request_hash, result_json FROM idempotency_records WHERE scope = 'research-brief' AND idempotency_key = $1",
      [input.runId],
    )
  ).rows[0];
  if (
    ...
    const cached = ResearchBriefSchema.safeParse(JSON.parse(saved.result_json));
    if (cached.success) return cached.data;
    const repaired = ResearchBriefSchema.parse(
      fallbackResearchBrief(input.question, input.profile),
    );
    await database.query(
      "UPDATE idempotency_records SET result_json = $1 WHERE scope = 'research-brief' AND idempotency_key = $2 AND request_hash = $3",
      [JSON.stringify(repaired), input.runId, requestHash],
    );
    return repaired;
  }
  const prompt = [
  ...
  try {
    ...
    await recordAuxiliaryCodexUsage(input.database, {
      runId: input.runId,
  ...
  await database.query(
    "INSERT INTO idempotency_records(scope, idempotency_key, request_hash, result_json, created_at) VALUES ('research-brief', $1, $2, $3, $4)",
    [input.runId, requestHash, JSON.stringify(brief), new Date().toISOString()],
  );
  return brief;
}
