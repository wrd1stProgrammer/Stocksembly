...
export async function readNewsCandidateClassifications(input: {
  ...
  if (
    (input.dataRoot === undefined && input.pool === undefined) ||
    input.candidateIds.length === 0
  )
    return [];
  const database = input.pool ?? (await getResearchPool());
  const { rows } = await database.query(
    `SELECT candidate_id, decision_status, classification_json
       FROM research.news_candidate_classifications
       WHERE symbol = $1 AND classifier_version = $2
         AND candidate_id = ANY($3::text[])`,
    [input.symbol, NEWS_CLASSIFIER_CACHE_VERSION, input.candidateIds],
  );
...
export async function writeNewsCandidateClassifications(input: {
  ...
  if (
    ...
    return;
  const database = input.pool ?? (await getResearchPool());
  const statement = `INSERT INTO research.news_candidate_classifications(
  ...
  await postgresTransaction(database, async (client) => {
    ...
    await client.query(
      "DELETE FROM research.news_candidate_classifications WHERE published_at < $1",
      [
        new Date(
...
export async function readNewsEventLedger(input: {
  ...
  if (input.dataRoot === undefined && input.pool === undefined) return [];
  const database = input.pool ?? (await getResearchPool());
  const { rows } = await database.query(
    `SELECT event_key, category, team_relevance_json, relevance,
        direction, horizon, verification_need, title, published_at,
        source, link, excerpt
       FROM research.news_events
       WHERE symbol = $1 AND published_at >= $2 AND published_at <= $3
       ORDER BY published_at DESC, relevance DESC`,
    [input.symbol, input.from, input.to],
  );
  return rows.flatMap((row) => {
    ...
    const event = Object.freeze({
      ...
      teamRelevance: Object.freeze([...decodeTeams(value.team_relevance_json)]),
...
export async function writeNewsEventLedger(input: {
  ...
  const statement = `INSERT INTO research.news_events(
  ...
  await postgresTransaction(database, async (client) => {
    ...
    await client.query(
      "DELETE FROM research.news_events WHERE published_at < $1",
      [
        new Date(
