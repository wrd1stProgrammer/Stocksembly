...
async function loadDepartmentResearchProfile(
  ...
  try {
    const row = (
      await database.query(
        `SELECT research_profile_json FROM research.research_requests WHERE run_id = $1`,
        [runId],
      )
    ).rows[0] as
      | {
          readonly research_profile_json?: unknown;
        }
      | undefined;
...
export async function publishDepartmentReportForRun(
  ...
  try {
    const run = RunSchema.safeParse(
      (
        await database.query(
          `SELECT runs.snapshot_id, runs.status, runs.version,
          ...
          FROM research.runs JOIN research.research_requests USING(run_id)
          WHERE runs.run_id = $1`,
          [runId],
        )
      ).rows[0],
    ...
    const rows = (
      await database.query(
        ...
        LEFT JOIN research.artifact_citation_metadata USING(artifact_id)
        LEFT JOIN research.agent_output_commits USING(artifact_id)
        WHERE artifacts.run_id = $1
          OR (artifacts.snapshot_id = $2 AND artifacts.logical_key LIKE 'evidence:%')`,
        [runId, run.data.snapshot_id],
      )
    ).rows.map((value) => ArtifactRowSchema.parse(value));
    ...
    let built = await buildReport(
      ...
      run.data,
      rows,
      await loadDepartmentResearchProfile(database, runId),
    );
    if (built === undefined)
    ...
    await researchTransaction(database, async (database) => {
      ...
      for (const row of built.parentRows)
        await database.query(
          ...
          [descriptor.artifactId, row.artifact_id],
        );
      await database.query(
        `INSERT INTO research.reports(report_id, run_id, snapshot_id,
            state, created_at) VALUES ($1, $2, $3, 'published', $4)`,
        [built.report.reportId, runId, run.data.snapshot_id, publishedAt],
      );
      const publicPayload = {
        schemaVersion: "workflow-v2",
      ...
      await database.query(
        `INSERT INTO research.report_versions(version_id, report_id, run_id,
            ...
            public_payload_json) VALUES ($1, $2, $3, $4, 1, $5, $6, $7, $8)`,
        [
      ...
      const changed = await database.query(
        `UPDATE research.runs SET status = 'complete-with-limitations',
            report_id = $1, report_published_at = $2, version = version + 1,
            last_event_seq = last_event_seq + 1
            WHERE run_id = $3 AND status = 'running' AND version = $4
              AND report_id IS NULL`,
        [built.report.reportId, publishedAt, runId, run.data.version],
      );
      ...
      const sequence = z
        ...
        .parse(
          (
            await database.query(
              `SELECT last_event_seq AS sequence FROM research.runs WHERE run_id = $1`,
              [runId],
            )
          ).rows[0],
        ).sequence;
      await database.query(
        `INSERT INTO research.run_events(run_id, sequence, event_id,
            event_type, state_id, occurred_at, payload_json)
            VALUES ($1, $2, $3, 'report_published', 'report-published', $4, $5)`,
        [
          runId,
          sequence,
          randomUUID(),
