...
export async function inspectAgentOutputBinding(
  database: ResearchDatabase,
  claim: AgentOutputInspectionClaim,
): Promise<unknown> {
  const value = (
    await database.query(
      `SELECT attempts.run_id, attempts.snapshot_id, attempts.job_id,
    ...
    JOIN research.agent_runner_evidence ON agent_runner_evidence.attempt_id = attempts.attempt_id
    LEFT JOIN research.agent_output_commits ON agent_output_commits.attempt_id = attempts.attempt_id
    WHERE attempts.run_id = $1 AND attempts.job_id = $2
      AND attempts.attempt_id = $3
      AND research_call_ordinals.ordinal = $4`,
      [claim.runId, claim.jobId, claim.attemptId, claim.ordinal],
    )
  ).rows[0];
  ...
  const citations = (
    ...
    FROM research.job_input_artifacts
    JOIN research.artifacts USING (artifact_id)
    JOIN research.artifact_citation_metadata USING (artifact_id)
    WHERE job_input_artifacts.job_id = $1
    UNION ALL
    SELECT artifacts.artifact_id AS artifact_id,
      artifacts.run_id AS run_id, artifacts.snapshot_id AS snapshot_id,
      artifacts.content_hash AS content_hash,
      artifact_citation_metadata.locator_json AS locator_json
    FROM research.attempt_web_evidence
    JOIN research.artifacts USING (artifact_id)
    JOIN research.artifact_citation_metadata USING (artifact_id)
    WHERE attempt_web_evidence.attempt_id = $2
      AND attempt_web_evidence.tool_transcript_hash = $3
    ORDER BY artifact_id`,
      [claim.jobId, claim.attemptId, row.runner_tool_transcript_hash],
    )
  ).rows.map((entry) => {
