...
export async function loadAuthenticatedReportSources(
  ...
  const rows = (
    await database.query(
      ...
      FROM research.artifacts LEFT JOIN research.agent_output_commits USING(artifact_id)
  ...
  for (const sourceId of sourceIds) {
    ...
    if (slice !== undefined) {
      ...
      sources.push({
        ...
        excerpt: slice.exactText.slice(0, 1500).trim(),
      });
      continue;
    }
    ...
    if (locator.success && locator.data.kind === "licensed_provider") {
      ...
      sources.push({
        ...
        url: locator.data.sourceUrl,
        excerpt: new TextDecoder().decode(stored.bytes).slice(0, 1500).trim(),
      });
      continue;
    }
