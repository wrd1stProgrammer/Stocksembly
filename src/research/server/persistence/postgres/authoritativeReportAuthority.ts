...
export async function loadReportAuthority(
...
) {
  {
    const runRow = (
      ...
      research_requests.locale AS locale FROM research.runs
      LEFT JOIN research.research_requests USING(run_id) WHERE run_id = $1`,
        [input.runId],
      )
    ).rows[0];
    const run =
    ...
    const rows = (
      ...
      agent_output_commits.envelope_json FROM research.artifacts
      LEFT JOIN research.agent_output_commits USING(artifact_id)
      LEFT JOIN research.attempts USING(attempt_id) WHERE artifacts.run_id = $1`,
        [input.runId],
      )
    ).rows.map((row) => RowSchema.parse(row));
    ...
    if (!structural.success) return blocked("structural_audit_invalid");
