...
async function setup() {
  ...
  return { database, store, runId };
}
async function advanceBeforeMandate(
  coordinator: Awaited<ReturnType<typeof createWorkflowCoordinator>>,
  runId: string,
) {
  for (const [index, eventKind] of (
    ...
    expect(
      (
        await coordinator.appendSystemTransition({
          runId,
...
describe("durable WorkflowV1 PostgreSQL coordinator", () => {
  it("atomically advances a system phase with its next jobs and public event", async () => {
    // Given
    const fixture = await setup();
    const coordinator = await createWorkflowCoordinator({
    ...
    for (const [index, eventKind] of (
      ...
      const result = await coordinator.appendSystemTransition({
        runId: fixture.runId,
    ...
    // Then
    expect(await fixture.store.findRun(fixture.runId)).toMatchObject({
      status: "running",
      lastEventSeq: 5,
    });
    expect(
      memos.every(
        async (job) => (await fixture.store.findJob(job.jobId)) !== undefined,
      ),
    ).toBe(true);
    expect(await coordinator.replay(fixture.runId)).toMatchObject({
      kind: "valid",
  ...
  it("rolls back both event and next jobs when one job conflicts", async () => {
    // Given
    const fixture = await setup();
    const coordinator = await createWorkflowCoordinator({
      database: fixture.database,
    });
    await advanceBeforeMandate(coordinator, fixture.runId);
    const memos = WORKFLOW_V1_SPECIALIST_IDS.map((roleId, index) => ({
    ...
    expect(await fixture.store.eventsAfter(fixture.runId, 0)).toHaveLength(4);
    expect((await fixture.store.findRun(fixture.runId))?.lastEventSeq).toBe(4);
    await coordinator.close();
    await fixture.store.close();
  });
  it("fails closed when durable workflow events were inserted out of order", async () => {
    const fixture = await setup();
    await fixture.store.appendRunEvent({
      runId: fixture.runId,
      event: {
    ...
    expect(await coordinator.replay(fixture.runId)).toEqual({
      kind: "invalid",
  ...
  it("rejects caller-selected jobs that do not match the transition contract", async () => {
    // Given
    const fixture = await setup();
    const coordinator = await createWorkflowCoordinator({
      database: fixture.database,
    ...
    expect(await fixture.store.eventsAfter(fixture.runId, 0)).toHaveLength(4);
    expect(
      await fixture.store.findJob(JobIdSchema.parse(uuid(96))),
    ).toBeUndefined();
    await coordinator.close();
    await fixture.store.close();
  });
  it("never resurrects a cancelling run through a late phase transition", async () => {
    // Given
    const fixture = await setup();
    ...
    expect((await fixture.store.findRun(fixture.runId))?.status).toBe(
      "cancelling",
    );
    await coordinator.close();
    await fixture.store.close();
  });
  it("replays workflow events across interleaved durable job events", async () => {
    // Given
    const fixture = await setup();
    const coordinator = await createWorkflowCoordinator({
      database: fixture.database,
