...
async function addDepartmentRequestAndPeers(
  ...
  await database.query(
    `INSERT INTO artifacts(artifact_id, run_id, snapshot_id, content_hash, byte_length, media_type, logical_key, input_hash, created_at) VALUES ($1, $2, $3, $4, $5, $6, 'evidence:insightsentry:peers', $7, $8)`,
    [
  ...
  await database.query(
    `INSERT INTO research_requests(run_id, principal_id, symbol, question, locale, request_hash, created_at, research_kind, department_id) VALUES ($1, $2, 'TSLA', 'Evaluate department evidence', 'en', $3, $4, 'department', $5)`,
    [
      runId,
      "a".repeat(64),
      "b".repeat(64),
...
it("publishes accepted/revised/removed adjudication without resurrecting removed claims", async () => {
  ...
  await round.stage({
    ...
    memberArtifactIds: (await round.acceptedMemos(runId)).map(
      (memo) => memo.artifactId,
    ),
  });
  ...
  const database = prepared.options.database;
  await database.query(
    `INSERT INTO research_requests(run_id, principal_id, symbol, question, locale, request_hash, created_at, research_kind, department_id) VALUES ($1, $2, 'TSLA', 'Evaluate market evidence', 'en', $3, $4, 'department', 'market')`,
    [runId, "a".repeat(64), "b".repeat(64), "2026-07-23T00:00:00.000Z"],
  );
  const publicationRow = (
    await database.query(
      `SELECT runs.snapshot_id, runs.status, runs.version, runs.report_id, research_requests.symbol, research_requests.question, research_requests.locale, research_requests.research_kind, research_requests.department_id FROM runs JOIN research_requests USING(run_id) WHERE runs.run_id = $1`,
      [runId],
    )
  ).rows[0];
  ...
  const result = await publishDepartmentReportForRun(
    { database: prepared.options.database, cas: prepared.options.cas },
    runId,
  );
  ...
  const persistedDatabase = prepared.options.database;
  const persistedPayload = JSON.parse(
    (
      (
        await persistedDatabase.query(
          `SELECT public_payload_json AS payload FROM report_versions`,
          [],
        )
      ).rows[0] as {
        payload: string;
...
it.each(["market", "company", "financial", "risk"] as const)(
  "publishes %s adjudication through real PostgreSQL/CAS with comparator lineage",
  async (departmentId) => {
