...
export function qualityMetricsForPublication(input: {
  readonly claims: readonly {
    readonly semanticVerdict?: string;
  }[];
...
export async function readResearchQualityObservation(
  database: ResearchDatabase,
  runId: string,
): Promise<ResearchQualityObservation | undefined> {
  const parsed = ObservationRowSchema.safeParse(
    (
      await database.query(
        `SELECT * FROM research.research_quality_observations WHERE run_id = $1`,
        [runId],
      )
    ).rows[0],
  );
...
export async function persistResearchQualityObservation(
  database: ResearchDatabase,
  observation: ResearchQualityObservation,
): Promise<void> {
  const current = await readResearchQualityObservation(
    database,
    observation.runId,
  );
  const outcome =
  ...
  await database.query(
    `INSERT INTO research.research_quality_observations(
