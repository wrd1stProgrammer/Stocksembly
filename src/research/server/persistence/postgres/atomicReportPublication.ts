    ...
    return await researchTransaction(database, async (database) => {
      ...
      const chair = ChairFenceRowSchema.parse(
        (
          ...
          FROM research.agent_output_commits JOIN research.attempts USING(attempt_id)
          WHERE agent_output_commits.artifact_id = $1`,
            [identity.acceptedChairArtifactId],
          )
        ).rows[0],
      );
      if (
      ...
      const run = RunRowSchema.parse(
        (
          await database.query(
            `SELECT snapshot_id, status, version, report_id, created_at
      ...
      const parentRows = (
        await database.query(
          `SELECT artifact_id, run_id, snapshot_id FROM research.artifacts WHERE artifact_id = ANY($1::text[])`,
          [identity.parentArtifactIds],
        )
      ...
      for (const parentId of identity.parentArtifactIds)
        ...
        child_artifact_id, parent_artifact_id, relation) VALUES ($1, $2, 'derived-from')`,
          [identity.descriptor.artifactId, parentId],
        );
      await database.query(
        `INSERT INTO research.reports(report_id, run_id, snapshot_id,
        state, created_at) VALUES ($1, $2, $3, 'draft', $4)`,
        [
      ...
      await database.query(
        `INSERT INTO research.report_versions(version_id, report_id,
      ...
      const changed = await database.query(
        `UPDATE research.runs SET status = $1,
        report_id = $2, report_published_at = $3, version = version + 1,
        last_event_seq = last_event_seq + 1 WHERE run_id = $4 AND status = 'running'
        AND version = $5 AND report_id IS NULL`,
        [
          terminalStatus,
          identity.version.reportId,
          input.commit.version.publishedAt,
      ...
      await database.query(
        ...
        $1, $2, $3, 'report_published', 'report-published', $4, $5)`,
        [
          identity.runId,
          sequence,
          identity.eventId,
