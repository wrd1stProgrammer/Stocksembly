ALTER TABLE research_call_ordinals RENAME TO research_call_ordinals_legacy_65;
...
SELECT run_id, ordinal, job_id, attempt_id, logical_artifact_key,
  input_hash, reserved_at, state
FROM research_call_ordinals_legacy_65;

DROP TABLE research_call_ordinals_legacy_65;

ALTER TABLE agent_output_commits RENAME TO agent_output_commits_legacy_65;
...
SELECT attempt_id, artifact_id, event_id, owner_id, fence_token,
  ordinal, output_hash, envelope_json, committed_at
FROM agent_output_commits_legacy_65;

DROP TABLE agent_output_commits_legacy_65;
...
BEGIN
  ...
  ) >= 3 THEN RAISE(ABORT, 'logical artifact replacement limit exceeded') END;
END;
