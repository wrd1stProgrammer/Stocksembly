ALTER TABLE research_call_ordinals RENAME TO research_call_ordinals_legacy_42;

CREATE TABLE research_call_ordinals (
  run_id TEXT NOT NULL REFERENCES runs(run_id) ON DELETE CASCADE,
  ordinal INTEGER NOT NULL CHECK (ordinal BETWEEN 1 AND 42),
...
SELECT run_id, ordinal, job_id, attempt_id, logical_artifact_key,
  input_hash, reserved_at, state
FROM research_call_ordinals_legacy_42;

DROP TABLE research_call_ordinals_legacy_42;

ALTER TABLE agent_output_commits RENAME TO agent_output_commits_legacy_42;
...
SELECT attempt_id, artifact_id, event_id, owner_id, fence_token,
  ordinal, output_hash, envelope_json, committed_at
FROM agent_output_commits_legacy_42;

DROP TABLE agent_output_commits_legacy_42;
