...
export const QUALITY_FATAL_REASONS = [
  ...
  "repeated_default_wait_posture",
  "direct_order_imperative",
  "raw_public_precision",
  "decisive_fact_not_entailed",
  "derived_input_ineligible",
  "exact_source_link_missing",
...
export const ResearchQualityFixtureSchema = z.object({
  ...
  metrics: z.array(MetricSchema).default([
    {
      ...
      registeredValue: 100,
      displayedValue: 100,
      declaredConversion: 1,
      precision: 2,
      displayedIssuer: "ACME",
      displayedPeriod: "FY2026",
      displayedUnit: "usd",
  ...
  sources: z
    .array(SourceSchema)
    .default([{ kind: "filing", purpose: "accounting_metric" }]),
  peerResult: z
    .object({
      emitted: z.boolean(),
    ...
    .default({
      emitted: true,
      rows: [{ qualified: true }, { qualified: true }, { qualified: true }],
    }),
  scenarios: z
    .array(z.object({ ordered: z.boolean(), assumptionsComplete: z.boolean() }))
    .default([{ ordered: true, assumptionsComplete: true }]),
  coreClaims: z
    .array(z.object({ assessable: z.boolean() }))
    .default([{ assessable: true }]),
  coreSections: z
    .array(z.string())
    .default(["Evidence supports a directional answer."]),
  publicTexts: z
    .array(z.string())
    .default(["Evidence supports a positive direction."]),
  precisionSamples: z
    .array(
      z.object({
        kind: z.enum(["currency", "percentage", "ratio"]),
        text: z.string(),
      }),
    )
    .default([]),
  decisiveFacts: z
    .array(z.object({ verdict: z.string(), exactSourceUrl: z.string() }))
    .default([
      { verdict: "entailed", exactSourceUrl: "https://example.com/source" },
    ]),
  decisiveAnalyses: z
    .array(
      z.object({
    ...
    .default([
      {
        verdict: "derived_supported",
        inputIds: ["fact-1"],
        eligibleInputIds: ["fact-1"],
      },
    ]),
  groundedCoreAnswer: z.boolean().default(true),
  wholeEnvelopeIntegrity: z.boolean().default(true),
  scoreEvidence: ScoreEvidenceSchema.default({
    directDirectionalAnswer: true,
    strongestCountercase: true,
    invalidationCondition: true,
    stanceInFirstSentence: true,
    limitationsBlockCount: 1,
    limitationsItemCount: 1,
    crossSectionThesisDuplicate: false,
...
export type ResearchQualityFixture = z.infer<
  typeof ResearchQualityFixtureSchema
>;
...
const WAIT_POSTURE =
  /\b(?:wait|conditional|needs? confirmation)\b|(?:기다|조건부|확인 필요)/iu;
...
export function evaluateResearchQuality(input: ResearchQualityFixture) {
  ...
  for (const metric of input.metrics) {
    if (
      metric.issuer !== input.targetIssuer ||
      metric.displayedIssuer !== metric.issuer
    )
      reasons.add("issuer_identity_mismatch");
    if (
      rounded(
        metric.registeredValue * metric.declaredConversion,
        metric.precision,
  ...
  if (
    input.sources.some((source) => !PURPOSES[source.kind].has(source.purpose))
  )
  ...
  if (
    input.scenarios.some(
      (scenario) => !scenario.ordered || !scenario.assumptionsComplete,
    )
  )
  ...
  if (
    input.publicTexts.some((text) => DIRECT_ORDER.test(text.normalize("NFKC")))
  )
    reasons.add("direct_order_imperative");
  if (
    input.precisionSamples.some(({ kind, text }) => {
  ...
  if (
    input.decisiveFacts.some(
      (fact) => !/^https:\/\/.+/u.test(fact.exactSourceUrl),
    )
  )
  ...
  const qualifiedPeerCount = input.peerResult.rows.filter(
    (row) => row.qualified,
  ).length;
  const peerOmissionValid = input.peerResult.emitted
    ? qualifiedPeerCount >= 3 &&
      qualifiedPeerCount === input.peerResult.rows.length
    : qualifiedPeerCount < 3;
  ...
  const scoreComponents = {
    ...
    editorialClarity:
      ...
      (evidence.limitationsBlockCount <= 1 && evidence.limitationsItemCount <= 3
        ? 0.5
        : 0) +
  ...
  return {
    ...
    scoreComponents,
    totalScore: Object.values(scoreComponents).reduce(
      (sum, value) => sum + value,
      0,
    ),
    runtimeDisposition: disposition(reasons),
    predicates: {
      directDirectionalAnswer: evidence.directDirectionalAnswer,
      strongestCountercase: evidence.strongestCountercase,
      ...
      peerOmissionValid,
      boundedPrecision,
      stanceInFirstSentence: evidence.stanceInFirstSentence,
      limitationsBlockCapped:
        evidence.limitationsBlockCount <= 1 &&
        evidence.limitationsItemCount <= 3,
      noCrossSectionThesisDuplication: !evidence.crossSectionThesisDuplicate,
      mixedQualityFailSoftCompletion: evidence.mixedQualityFailSoftCompletion,
      validScenarios,
      sourceLocaleCacheContract: evidence.sourceLocaleCacheContract,
      atomicPublication: evidence.atomicPublication,
    },
  };
}
