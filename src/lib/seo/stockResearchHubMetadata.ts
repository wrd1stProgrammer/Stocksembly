...
import { boundedSeoDescription, brandedSeoTitle } from "./metadataText";
...
export function stockResearchHubMetadata(
  ...
  const title = brandedSeoTitle(
    uiMessage(locale, {
      en: `${hub.company} (${hub.symbol}) Stock Analysis`,
      ko: `${hub.company}(${hub.symbol}) 미국주식 분석`,
      ja: `${hub.company}（${hub.symbol}）米国株分析`,
      "zh-TW": `${hub.company}（${hub.symbol}）美股分析`,
      es: `Análisis de ${hub.company} (${hub.symbol})`,
      "pt-BR": `Análise de ${hub.company} (${hub.symbol})`,
      de: `${hub.company} (${hub.symbol}) Aktienanalyse`,
      fr: `Analyse de ${hub.company} (${hub.symbol})`,
    }),
  );
