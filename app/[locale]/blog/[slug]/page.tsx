...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    editorialEntries("blog").map(({ slug }) => ({ locale, slug })),
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return isLocale(locale) && definition?.kind === "blog"
...
export default async function BlogArticlePage({ params }: Props) {
  const { locale, slug } = await params;
  const definition = editorialDefinition(slug);
  if (!isLocale(locale) || definition?.kind !== "blog") notFound();
  const access = await editorialWorkspaceAccess(
    editorialPath(locale, "blog", definition.slug),
  );
  return (
    <EditorialWorkspaceShell
      ...
      activeItem="blog"
      locale={locale}
      localePaths={editorialLocalePaths("blog", definition.slug)}
    >
