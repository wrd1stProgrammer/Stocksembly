> Historical design record preserved from commit `47b0b22782ddd043abd6b987f4f2e7b5b51d77b5`. Paths below are relative to the repository root at that revision. These iterations are not a list of currently deployed requirements. Use [DESIGN.md](../../DESIGN.md) for current entry points.
...
- Credit allocation: Free receives a one-time 5-credit sign-up grant; Pro receives 150 credits per month; Ultra receives 400 credits per month. The allowance is a visible value row on each card, while the feature list explains the outcome of the plan.
- Usage policy: selecting research options never consumes credits; full-agent research consumes 10 credits after successful completion, department-agent research consumes 5 credits after successful completion, every 100 chat messages consumes 10 credits, opening a research-room report less than seven days old consumes 3 credits, and professional translation consumes 1 credit. Reports at least seven days old are free to open. Chat message counts are server-side and failed research does not consume credits.
- Plan copy: Free communicates the one-time 5-credit starting balance; Pro communicates selectable research options, free access to research after seven days, and daily briefings for three watchlist names; Ultra communicates ten-name daily briefings and early access to new features.
...
- Structure: one account-scoped, four-step dialog appears for every authenticated account that has not completed the current onboarding version. It asks which investment decision the member wants to clarify, records an acquisition survey response, explains the exact credit cost of each core action, and closes with the shared Free, Pro, and Ultra pricing grid.
- Persistence: completion is stored as a versioned account field together with the acquisition source. The migration marks existing accounts as pending so members who have not seen this flow receive it after their next sign-in. There is no skip or close action; choosing Free, selecting a paid plan, or opening the plan comparison completes the flow.
- Surface: the dialog uses the existing midnight evidence-room tokens, compact balanced headings, radio-style choices, cobalt actions, and restrained atmospheric light. Its final step reuses the subscription-management monthly/annual toggle and plan cards so pricing stays consistent across the product.
- Accessibility and responsive state: native buttons expose pressed, disabled, and focus states; the modal locks page scroll and remains full-height on narrow screens. Every supported application locale receives matching content and the cost values always come from the billing policy constants.
- Integration: free is a local access state; paid cards receive sanitized plan data and checkout URLs from `/api/billing/plans`. Whop secrets remain server-only so the sandbox-to-production switch is environment configuration, not client code. A hidden `$1/month` production-verification plan is available only through the admin checkout route and maps to the same Pro entitlement as the public `$19/month` plan.
...
- Every analyst owns two typed seated anchors: a personal `workSeat` at a PC for independent research and a `meetingSeat` at the department table for team consensus. Both anchors stay on the top or bottom edge of their furniture; side chairs and side-facing seated poses are prohibited. Dr. Park keeps one chair desk and does not duplicate seats.
- Personal work seats use `(seat)/(input)/facing`: Maya `(4,14)/(4,15)/down`; June `(8,14)/(8,15)/down`; Alex `(12,14)/(12,15)/down`; Ethan `(31,14)/(31,15)/down`; Aria `(35,14)/(35,15)/down`; Leo `(39,14)/(39,15)/down`; Noah `(5,31)/(5,32)/down`; Sofia `(9,31)/(9,32)/down`; Hana `(13,31)/(13,32)/down`; Liam `(31,31)/(31,32)/down`; Min `(35,31)/(35,32)/down`. Navigation retains these evenly spaced cells. Each rendered chair and seated actor aligns with the center of its own table, while the workstation bitmap is visually lowered by `8px` and its separate chair and actor are lowered by `16px` to sit tight against the desk. Meeting seats retain the current table composition: Maya `(8,7)/(8,8)/down`; June `(7,11)/(7,10)/up`; Alex `(9,11)/(9,10)/up`; Ethan `(35,7)/(35,8)/down`; Aria `(34,11)/(34,10)/up`; Leo `(37,11)/(37,10)/up`; Noah `(7,24)/(7,25)/down`; Sofia `(11,24)/(11,25)/down`; Hana `(9,28)/(9,27)/up`; Liam `(31,24)/(31,25)/down`; Min `(34,28)/(34,27)/up`. Dr. Park remains `(22,11)/(22,10)/up`.
- Individual-research bubbles originate from the analyst's personal PC seat. Department checkpoints route every department member to the shared meeting table, settle them into their `meetingSeat`, and use seated talk/listen poses. Standing talk anchors are reserved for cross-team visits: Market lead `(12,11)`, Company lead `(31,10)`, Financial lead `(13,24)`, and Risk lead `(30,23)`.
...
- Department work is visibly concurrent. Independent research keeps teammates at personal PC workstations; the department checkpoint visibly moves them to the meeting table before seated consensus begins. Visitor anchors preserve host/visitor facing; forum anchors turn Maya, Ethan, Noah, Liam, and Dr. Park toward `(24,14)`.
- At completion exactly five actors remain standing in the forum and six non-representatives remain at their personal work seats. Personal and meeting chairs exist from the first frame; occupancy changes its subtle highlight only.
...
- Parallel research starts visibly at tick 40 with department-separated individual progress bubbles from the analysts seated at their own PCs. The transition into tick 240 first walks every department member to the shared table; team consensus bubbles begin only after the relevant speaker has reached the meeting seat. Later bubbles are owned by public `talk`, `summarize`, `present`, and `chair-synthesis` actions, never hidden chain-of-thought.
...
- Department furniture is generated from the same manifest as sixteen independent Pixi clusters: eleven compact image-generated one-person tables, four procedural meeting tables, and Dr. Park's chair desk. The personal tables sit at even intervals along the lower band of each department in the actual `3/3/3/2` headcount. All eleven instances reuse one minimal table-only transparent bitmap; it contains no laptop, monitor, chair, person, room, shadow, or decorative clutter. Each personal table receives the same procedural laptop used on the central meeting tables and one existing direction-specific chair sprite. Every analyst sits immediately north of their own table with `facing: down`, keeping the front-facing seated frame visible. Furniture entries declare `purpose: workstation | meeting | chair`, and a personal workstation is bound to exactly one analyst. Furniture footprints are physical obstacles; every personal seat, meeting seat, door, visitor anchor, and cross-room route remains walkable and unobstructed.
...
| `40..239` | four departments conduct individual research at personal PC workstations |
| `240..359` | every team moves to its meeting table, then holds seated consensus |
...
### Editorial library: Blog and Glossary

The public editorial library translates the attached reference composition into the Midnight Evidence Room system. The source references are the four user-provided captures: the centered three-column index, the article hero with one dominant image, the narrow long-form body with a closing action, and the three-card read-next rail. They define composition only; typography, color, imagery, and interaction remain Stocksembly-native.

- `/[locale]/blog` and `/[locale]/glossary` are server-rendered discovery pages. Each begins with a centered eyebrow, one direct search-intent headline, a restrained description, and a responsive card grid. Cards use one generated 16:9 editorial image, compact type/date metadata, a strong two-to-three-line title, and a muted excerpt.
- `/[locale]/blog/[slug]` and `/[locale]/glossary/[slug]` are reading surfaces. A quiet back link precedes metadata, an editorial display title, a short deck, and one dominant 16:9 image. The body uses a narrow `42rem` reading measure, explicit section hierarchy, generous paragraph rhythm, and a single evidence-colored closing action.
- The related-content section repeats the same card primitive in a three-column row so the visual grammar does not change between discovery and continuation.
- Generated imagery is conceptual institutional still life: near-black graphite environments, cobalt evidence light, restrained amber risk accents, realistic glass/metal/paper, and no baked text, logos, tickers, people, or invented performance claims.
- Authenticated editorial surfaces keep the same workspace shell as Research Room and Briefing Room: the `48px` collapsed rail is visible on entry and may be expanded by the user. Anonymous readers see only the quiet Stocksembly masthead. Editorial routes never duplicate language, authentication, or sign-out controls in the upper-right corner; authenticated locale and account actions stay inside the shared sidebar.
- Editorial surfaces reuse the footer, Inter/Pretendard stack, focus treatment, and existing color tokens. Cards do not add ornamental gradients or excessive floating containers; hierarchy comes from image scale, spacing, borders, and type weight.
- Desktop discovery uses the reference's compact editorial proportions rather than the landing-page display scale: a `min(1180px, viewport - gutters)` grid, `12px` card radius, `18–20px` card padding, `0.98–1.2rem` card titles, and a hero capped around `3.8rem`. Reading pages cap the article hero at `820px`, the reading measure at `40rem`, the detail title at `3.7rem`, and section headings at `1.8rem`; the 16:9 image remains dominant but never becomes a full-width billboard.
- Editorial cards use the exact Research Room interaction grammar: the shared monochrome `BorderBeam` activates on hover, focus, and pointer-down while the inner surface shifts to `#111318`, its border moves to `#393d46`, and the whole card rises only `2px`. Image scale is restrained to preserve the reference's calm card rhythm.
