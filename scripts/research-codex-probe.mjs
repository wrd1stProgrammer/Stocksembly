...
const entryPath = resolve(
  projectRoot,
  "src/research/server/codex/readinessCommand.ts",
);
let buildRoot;

try {
  await mkdir(buildParent, { recursive: true });
  buildRoot = await mkdtemp(join(buildParent, "codex-readiness-cli-"));
  ...
  if (errors.length > 0) throw new Error("CODEX_READINESS_BUILD_FAILED");
  ...
  if (typeof commandModule.runReadinessCommand !== "function")
    throw new Error("CODEX_READINESS_BUILD_FAILED");
  process.exitCode = await commandModule.runReadinessCommand();
} catch {
  process.stderr.write(
    '{"status":"blocked","code":"CODEX_ISOLATION_FAILED","check":"probe"}\n',
  );
  process.exitCode = 1;
} finally {
