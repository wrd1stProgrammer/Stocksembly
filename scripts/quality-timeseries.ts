// Recalculate published report quality from a PostgreSQL publication snapshot and
...
import { readResearchQualityObservation } from "../src/research/server/persistence/postgres/researchQualityObservations";
...
async function main(database: Pool): Promise<void> {
  ...
  const maskedDatabasePath = "PostgreSQL publication snapshot";
  ...
  ).rows.map((row) => ReportVersionRowSchema.parse(row));
  ...
  for (const row of rows) {
    try {
      ...
      const observation = await readResearchQualityObservation(
        database,
        row.run_id,
      );
  ...
  const archive = {
    ...
    generatedAt,
    sourcePublicationSnapshot: `sha256:${sourceDbSnapshot}`,
...
await withResearchAuditDatabase(main);
